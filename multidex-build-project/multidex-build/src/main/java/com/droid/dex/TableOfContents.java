...
public final class TableOfContents {
    ...
    public void readFrom(com.droid.dex.Dex dex) throws IOException {
        readHeader(dex.open(0));
        readMap(dex.open(mapList.off));
        computeSizesFromOffsets();
    }

    private void readHeader(com.droid.dex.Dex.Section headerIn) throws UnsupportedEncodingException {
        byte[] magic = headerIn.readByteArray(8);

        if (!com.droid.dex.DexFormat.isSupportedDexMagic(magic)) {
            throw new DexException("Unexpected magic: " + Arrays.toString(magic));
        }

        apiLevel = com.droid.dex.DexFormat.magicToApi(magic);
        checksum = headerIn.readInt();
        signature = headerIn.readByteArray(20);
        fileSize = headerIn.readInt();
        int headerSize = headerIn.readInt();
        if (headerSize != com.droid.dex.SizeOf.HEADER_ITEM) {
            throw new DexException("Unexpected header: 0x" + Integer.toHexString(headerSize));
        }
        int endianTag = headerIn.readInt();
        if (endianTag != com.droid.dex.DexFormat.ENDIAN_TAG) {
    ...
    private void readMap(com.droid.dex.Dex.Section in) throws IOException {
    ...
    public void writeHeader(com.droid.dex.Dex.Section out, int api) throws IOException {
        out.write(com.droid.dex.DexFormat.apiToMagic(api).getBytes("UTF-8"));
        out.writeInt(checksum);
        out.write(signature);
        out.writeInt(fileSize);
        out.writeInt(com.droid.dex.SizeOf.HEADER_ITEM);
        out.writeInt(com.droid.dex.DexFormat.ENDIAN_TAG);
    ...
    public void writeMap(com.droid.dex.Dex.Section out) throws IOException {
        int count = 0;
        for (Section section : sections) {
            if (section.exists()) {
                count++;
