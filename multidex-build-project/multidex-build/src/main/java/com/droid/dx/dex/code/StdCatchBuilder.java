...
/**
 * Constructor of {@link CatchTable} instances from {@link com.droid.dx.rop.code.RopMethod}
 * and associated data.
 */
public final class StdCatchBuilder implements CatchBuilder {
     ...
     */
    public StdCatchBuilder(com.droid.dx.rop.code.RopMethod method, int[] order,
    ...
    public boolean hasAnyCatches() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            com.droid.dx.rop.type.TypeList catches = block.getLastInsn().getCatches();
            if (catches.size() != 0) {
                return true;
            }
        }
    ...
    /** {@inheritDoc} */
    public HashSet<com.droid.dx.rop.type.Type> getCatchTypes() {
        HashSet<com.droid.dx.rop.type.Type> result = new HashSet<com.droid.dx.rop.type.Type>(20);
        com.droid.dx.rop.code.BasicBlockList blocks = method.getBlocks();
        int size = blocks.size();
     ...
     */
    public static CatchTable build(com.droid.dx.rop.code.RopMethod method, int[] order,
                                   BlockAddresses addresses) {
    ...
    private static CatchHandlerList handlersFor(BasicBlock block,
        ...
        for (int i = 0; i < catchSize; i++) {
            com.droid.dx.rop.type.Type type = catches.getType(i);
            if (type.equals(com.droid.dx.rop.type.Type.OBJECT)) {
                catchSize = i + 1;
                break;
            }
        }
        ...
        for (int i = 0; i < catchSize; i++) {
            com.droid.dx.rop.cst.CstType oneType = new com.droid.dx.rop.cst.CstType(catches.getType(i));
