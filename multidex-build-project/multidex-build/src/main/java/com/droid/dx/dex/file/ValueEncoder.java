...
public final class ValueEncoder {
    ...
    public void writeConstant(Constant cst) {
        ...
        switch (type) {
            ...
            case VALUE_LONG: {
                ...
                com.droid.dex.EncodedValueCodec.writeSignedIntegralValue(out, type, value);
                break;
            }
            case VALUE_CHAR: {
                long value = ((CstLiteralBits) cst).getLongBits();
                com.droid.dex.EncodedValueCodec.writeUnsignedIntegralValue(out, type, value);
                break;
            }
            case VALUE_FLOAT: {
                // Shift value left 32 so that right-zero-extension works.
                long value = ((com.droid.dx.rop.cst.CstFloat) cst).getLongBits() << 32;
                com.droid.dex.EncodedValueCodec.writeRightZeroExtendedValue(out, type, value);
                break;
            }
            case VALUE_DOUBLE: {
                long value = ((com.droid.dx.rop.cst.CstDouble) cst).getLongBits();
                com.droid.dex.EncodedValueCodec.writeRightZeroExtendedValue(out, type, value);
                break;
            }
            case VALUE_STRING: {
                int index = file.getStringIds().indexOf((com.droid.dx.rop.cst.CstString) cst);
                com.droid.dex.EncodedValueCodec.writeUnsignedIntegralValue(out, type, (long) index);
                break;
            }
            case VALUE_TYPE: {
            ...
            case VALUE_METHOD: {
                int index = file.getMethodIds().indexOf((com.droid.dx.rop.cst.CstMethodRef) cst);
                com.droid.dex.EncodedValueCodec.writeUnsignedIntegralValue(out, type, (long) index);
                break;
            }
            case VALUE_ENUM: {
                CstFieldRef fieldRef = ((com.droid.dx.rop.cst.CstEnumRef) cst).getFieldRef();
            ...
            case VALUE_ANNOTATION: {
                out.writeByte(type);
                writeAnnotation(((com.droid.dx.rop.cst.CstAnnotation) cst).getAnnotation(),
                        false);
                break;
            }
            case VALUE_NULL: {
            ...
            case VALUE_BOOLEAN: {
                int value = ((com.droid.dx.rop.cst.CstBoolean) cst).getIntBits();
    ...
    private static int constantToValueType(Constant cst) {
        ...
        if (cst instanceof CstByte) {
            ...
            return VALUE_CHAR;
        } else if (cst instanceof com.droid.dx.rop.cst.CstInteger) {
            return VALUE_INT;
        } else if (cst instanceof com.droid.dx.rop.cst.CstLong) {
            return VALUE_LONG;
        } else if (cst instanceof com.droid.dx.rop.cst.CstFloat) {
            return VALUE_FLOAT;
        } else if (cst instanceof com.droid.dx.rop.cst.CstDouble) {
            return VALUE_DOUBLE;
        } else if (cst instanceof com.droid.dx.rop.cst.CstString) {
            return VALUE_STRING;
        } else if (cst instanceof CstType) {
            return VALUE_TYPE;
        } else if (cst instanceof CstFieldRef) {
            return VALUE_FIELD;
        } else if (cst instanceof com.droid.dx.rop.cst.CstMethodRef) {
            return VALUE_METHOD;
        } else if (cst instanceof com.droid.dx.rop.cst.CstEnumRef) {
            return VALUE_ENUM;
        } else if (cst instanceof CstArray) {
            return VALUE_ARRAY;
        } else if (cst instanceof com.droid.dx.rop.cst.CstAnnotation) {
            return VALUE_ANNOTATION;
        } else if (cst instanceof CstKnownNull) {
            return VALUE_NULL;
        } else if (cst instanceof com.droid.dx.rop.cst.CstBoolean) {
    ...
    public void writeAnnotation(Annotation annotation, boolean topLevel) {
        ...
        for (NameValuePair pair : pairs) {
            com.droid.dx.rop.cst.CstString name = pair.getName();
     ...
     * encounter a {@link com.droid.dx.rop.cst.CstAnnotation}.
     *
     * @param file {@code non-null;} the file to add to
     * @param cst {@code non-null;} the constant to add contents for
     */
    public static void addContents(DexFile file, Constant cst) {
        if (cst instanceof com.droid.dx.rop.cst.CstAnnotation) {
            addContents(file, ((com.droid.dx.rop.cst.CstAnnotation) cst).getAnnotation());
        } else if (cst instanceof CstArray) {
