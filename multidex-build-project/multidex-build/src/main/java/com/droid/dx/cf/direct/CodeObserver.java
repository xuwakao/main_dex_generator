...
public class CodeObserver implements BytecodeArray.Visitor {
     ...
     */
    public CodeObserver(ByteArray bytes, com.droid.dx.cf.iface.ParseObserver observer) {
        if (bytes == null) {
    ...
    public void visitLocal(int opcode, int offset, int length,
        ...
        if (opcode == com.droid.dx.cf.code.ByteOps.IINC) {
            valueStr = ", #" +
    ...
    public void visitConstant(int opcode, int offset, int length,
        ...
        if (cst instanceof com.droid.dx.rop.cst.CstInteger) {
            visitLiteralInt(opcode, offset, length, value);
            return;
        }

        if (cst instanceof com.droid.dx.rop.cst.CstLong) {
            visitLiteralLong(opcode, offset, length,
                             ((com.droid.dx.rop.cst.CstLong) cst).getValue());
            return;
        }

        if (cst instanceof com.droid.dx.rop.cst.CstFloat) {
            visitLiteralFloat(opcode, offset, length,
                              ((com.droid.dx.rop.cst.CstFloat) cst).getIntBits());
            return;
        }

        if (cst instanceof com.droid.dx.rop.cst.CstDouble) {
            visitLiteralDouble(opcode, offset, length,
                             ((com.droid.dx.rop.cst.CstDouble) cst).getLongBits());
            return;
        }
    ...
    public void visitSwitch(int opcode, int offset, int length,
                            com.droid.dx.cf.code.SwitchList cases, int padding) {
        int sz = cases.size();
    ...
    private String header(int offset) {
        ...
        String name = com.droid.dx.cf.code.ByteOps.opName(opcode);

        if (opcode == com.droid.dx.cf.code.ByteOps.WIDE) {
            opcode = bytes.getUnsignedByte(offset + 1);
            name += " " + com.droid.dx.cf.code.ByteOps.opName(opcode);
        }
    ...
    private void visitLiteralInt(int opcode, int offset, int length,
        ...
        if ((length == 1) || (opcode == com.droid.dx.cf.code.ByteOps.BIPUSH)) {
            valueStr = "#" + Hex.s1(value);
        } else if (opcode == com.droid.dx.cf.code.ByteOps.SIPUSH) {
