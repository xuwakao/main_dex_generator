...
public class EscapeAnalysis {
    ...
    private void processInsn(SsaInsn insn) {
        ...
        if (op == com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO &&
        ...
        } else if (op == com.droid.dx.rop.code.RegOps.MOVE_PARAM &&
                      result.getTypeBearer().getBasicType() == Type.BT_OBJECT) {
            ...
            processRegister(result, escSet);
        } else if (op == com.droid.dx.rop.code.RegOps.MOVE_RESULT &&
                result.getTypeBearer().getBasicType() == Type.BT_OBJECT) {
    ...
    private EscapeSet processMoveResultPseudoInsn(SsaInsn insn) {
        ...
        switch(prevOpcode) {
           // New instance / Constant
            case com.droid.dx.rop.code.RegOps.NEW_INSTANCE:
            case com.droid.dx.rop.code.RegOps.CONST:
                escSet = new EscapeSet(result.getReg(), regCount,
                                           EscapeState.NONE);
                break;
            // New array
            case com.droid.dx.rop.code.RegOps.NEW_ARRAY:
            case com.droid.dx.rop.code.RegOps.FILLED_NEW_ARRAY:
            ...
            case com.droid.dx.rop.code.RegOps.GET_STATIC:
                escSet = new EscapeSet(result.getReg(), regCount,
                                           EscapeState.GLOBAL);
                break;
            // Type cast / load an object from a field or array
            case com.droid.dx.rop.code.RegOps.CHECK_CAST:
            case com.droid.dx.rop.code.RegOps.GET_FIELD:
            case com.droid.dx.rop.code.RegOps.AGET:
                prevSource = prevSsaInsn.getSources().get(0);
    ...
    private void processUse(RegisterSpec def, SsaInsn use, EscapeSet escSet,
        ...
        switch (useOpcode) {
            case com.droid.dx.rop.code.RegOps.MOVE:
                // Follow uses of the move by adding it to the worklist
                escSet.regSet.set(use.getResult().getReg());
                regWorklist.add(use.getResult());
                break;
            case com.droid.dx.rop.code.RegOps.IF_EQ:
            case com.droid.dx.rop.code.RegOps.IF_NE:
            case com.droid.dx.rop.code.RegOps.CHECK_CAST:
                ...
                break;
            case com.droid.dx.rop.code.RegOps.APUT:
            ...
            case com.droid.dx.rop.code.RegOps.PUT_FIELD:
            ...
            case com.droid.dx.rop.code.RegOps.AGET:
                ...
                break;
            case com.droid.dx.rop.code.RegOps.PUT_STATIC:
                // Static puts cause an object to escape globally
                escSet.escape = EscapeState.GLOBAL;
                break;
            case com.droid.dx.rop.code.RegOps.INVOKE_STATIC:
            case com.droid.dx.rop.code.RegOps.INVOKE_VIRTUAL:
            case com.droid.dx.rop.code.RegOps.INVOKE_SUPER:
            case com.droid.dx.rop.code.RegOps.INVOKE_DIRECT:
            case com.droid.dx.rop.code.RegOps.INVOKE_INTERFACE:
            case com.droid.dx.rop.code.RegOps.RETURN:
            case com.droid.dx.rop.code.RegOps.THROW:
    ...
    private void replaceDef(SsaInsn def, SsaInsn prev, int length,
        ...
        for (int i = 0; i < length; i++) {
            Constant newZero = com.droid.dx.rop.cst.Zeroes.zeroFor(resultType.getComponentType());
            ...
            insertPlainInsnBefore(def, RegisterSpecList.EMPTY, newReg,
                                      com.droid.dx.rop.code.RegOps.CONST, newZero);
        }
    }
    ...
    private void replaceUse(SsaInsn use, SsaInsn prev,
        ...
        switch (use.getOpcode().getOpcode()) {
            case com.droid.dx.rop.code.RegOps.AGET:
                ...
                if (index < length) {
                    ...
                    insertPlainInsnBefore(next, RegisterSpecList.make(source),
                                              result, com.droid.dx.rop.code.RegOps.MOVE, null);
                } else {
            ...
            case com.droid.dx.rop.code.RegOps.APUT:
                ...
                break;
            case com.droid.dx.rop.code.RegOps.ARRAY_LENGTH:
                ...
                insertPlainInsnBefore(next, RegisterSpecList.EMPTY,
                                          next.getResult(), com.droid.dx.rop.code.RegOps.CONST,
                                          (Constant) lengthReg);
                deletedInsns.add(next);
                break;
            case com.droid.dx.rop.code.RegOps.MARK_LOCAL:
                // Remove mark local instructions
                break;
            case com.droid.dx.rop.code.RegOps.FILL_ARRAY_DATA:
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    insertPlainInsnBefore(use, RegisterSpecList.EMPTY, newFill,
                                              com.droid.dx.rop.code.RegOps.CONST, constList.get(i));
    ...
    private void movePropagate() {
        for (int i = 0; i < ssaMeth.getRegCount(); i++) {
            ...
            if (insn == null || insn.getOpcode() == null ||
                insn.getOpcode().getOpcode() != com.droid.dx.rop.code.RegOps.MOVE) {
                continue;
            }
    ...
    private void insertExceptionThrow(SsaInsn insn, RegisterSpec index,
        ...
        CstType exception =
            new CstType(com.droid.dx.rop.code.Exceptions.TYPE_ArrayIndexOutOfBoundsException);
        insertThrowingInsnBefore(insn, RegisterSpecList.EMPTY, null,
                                     com.droid.dx.rop.code.RegOps.NEW_INSTANCE, exception);
        ...
        insertPlainInsnBefore(newInsn, RegisterSpecList.EMPTY, newReg,
                                  com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO, null);
        ...
        com.droid.dx.rop.cst.CstNat newNat = new com.droid.dx.rop.cst.CstNat(new com.droid.dx.rop.cst.CstString("<init>"), new com.droid.dx.rop.cst.CstString("(I)V"));
        com.droid.dx.rop.cst.CstMethodRef newRef = new com.droid.dx.rop.cst.CstMethodRef(exception, newNat);
        insertThrowingInsnBefore(newInsn2, RegisterSpecList.make(newReg, index),
                                     null, com.droid.dx.rop.code.RegOps.INVOKE_DIRECT, newRef);
        ...
        insertThrowingInsnBefore(newInsn3, RegisterSpecList.make(newReg), null,
                                     com.droid.dx.rop.code.RegOps.THROW, null);
    ...
    private void insertPlainInsnBefore(SsaInsn insn,
        ...
        Rop newRop;
        if (newOpcode == com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
            newRop = com.droid.dx.rop.code.Rops.opMoveResultPseudo(newResult.getType());
        } else {
            newRop = com.droid.dx.rop.code.Rops.ropFor(newOpcode, newResult, newSources, cst);
        }
    ...
    private void insertThrowingInsnBefore(SsaInsn insn,
        ...
        Rop newRop = com.droid.dx.rop.code.Rops.ropFor(newOpcode, newResult, newSources, cst);
        Insn newRopInsn;
        if (cst == null) {
            newRopInsn = new com.droid.dx.rop.code.ThrowingInsn(newRop,
                origRopInsn.getPosition(), newSources, com.droid.dx.rop.type.StdTypeList.EMPTY);
        } else {
            newRopInsn = new ThrowingCstInsn(newRop,
                origRopInsn.getPosition(), newSources, com.droid.dx.rop.type.StdTypeList.EMPTY, cst);
        }
