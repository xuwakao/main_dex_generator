...
public final class DexIndexPrinter {
    ...
    private final com.droid.dex.TableOfContents tableOfContents;

    public DexIndexPrinter(File file) throws IOException {
        this.dex = new com.droid.dex.Dex(file);
        this.tableOfContents = dex.getTableOfContents();
    }

    private void printMap() {
        for (com.droid.dex.TableOfContents.Section section : tableOfContents.sections) {
            if (section.off != -1) {
    ...
    private void printProtoIds() throws IOException {
        int index = 0;
        for (com.droid.dex.ProtoId protoId : dex.protoIds()) {
    ...
    private void printFieldIds() throws IOException {
        int index = 0;
        for (com.droid.dex.FieldId fieldId : dex.fieldIds()) {
    ...
    private void printMethodIds() throws IOException {
        int index = 0;
        for (com.droid.dex.MethodId methodId : dex.methodIds()) {
    ...
    private void printTypeLists() throws IOException {
        ...
        com.droid.dex.Dex.Section in = dex.open(tableOfContents.typeLists.off);
