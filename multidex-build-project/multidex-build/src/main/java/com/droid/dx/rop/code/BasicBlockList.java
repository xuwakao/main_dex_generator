...
import com.droid.dx.util.LabeledList;

/**
 * List of {@link com.droid.dx.rop.code.BasicBlock} instances.
 */
public final class BasicBlockList extends LabeledList {
     ...
     */
    public com.droid.dx.rop.code.BasicBlock get(int n) {
        return (com.droid.dx.rop.code.BasicBlock) get0(n);
    }
     ...
     */
    public void set(int n, com.droid.dx.rop.code.BasicBlock bb) {
        super.set(n, bb);
     ...
     * instance's instructions (indirectly through {@link com.droid.dx.rop.code.BasicBlock}
     * instances).
     *
    ...
    public int getInstructionCount() {
        ...
        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.BasicBlock one = (com.droid.dx.rop.code.BasicBlock) getOrNull0(i);
            if (one != null) {
        ...
        for (int i = 0; i < sz; i++) {
            ...
            if (one != null) {
                ...
                for (int j = 0; j < insnsSz; j++) {
                    com.droid.dx.rop.code.Insn insn = insns.get(j);
     ...
     */
    public com.droid.dx.rop.code.BasicBlock labelToBlock(int label) {
     ...
     */
    public void forEachInsn(com.droid.dx.rop.code.Insn.Visitor visitor) {
        int sz = size();

        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.BasicBlock one = get(i);
            InsnList insns = one.getInsns();
    ...
    public BasicBlockList withRegisterOffset(int delta) {
        ...
        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.BasicBlock one = (com.droid.dx.rop.code.BasicBlock) get0(i);
            if (one != null) {
     ...
     */
    public com.droid.dx.rop.code.BasicBlock preferredSuccessorOf(com.droid.dx.rop.code.BasicBlock block) {
     ...
     * are identical
     */
    public boolean catchesEqual(com.droid.dx.rop.code.BasicBlock block1, com.droid.dx.rop.code.BasicBlock block2) {
        com.droid.dx.rop.type.TypeList catches1 = block1.getExceptionHandlerTypes();
        com.droid.dx.rop.type.TypeList catches2 = block2.getExceptionHandlerTypes();

        if (!com.droid.dx.rop.type.StdTypeList.equalContents(catches1, catches2)) {
            return false;
        }
    ...
    private static class RegCountVisitor
            implements com.droid.dx.rop.code.Insn.Visitor {
        ...
        /** {@inheritDoc} */
        public void visitPlainInsn(com.droid.dx.rop.code.PlainInsn insn) {
            visit(insn);
        }
        ...
        /** {@inheritDoc} */
        public void visitFillArrayDataInsn(com.droid.dx.rop.code.FillArrayDataInsn insn) {
            visit(insn);
        }
         ...
         */
        private void visit(com.droid.dx.rop.code.Insn insn) {
            com.droid.dx.rop.code.RegisterSpec result = insn.getResult();

            if (result != null) {
                processReg(result);
            }

            com.droid.dx.rop.code.RegisterSpecList sources = insn.getSources();
            int sz = sources.size();
         ...
         */
        private void processReg(com.droid.dx.rop.code.RegisterSpec spec) {
