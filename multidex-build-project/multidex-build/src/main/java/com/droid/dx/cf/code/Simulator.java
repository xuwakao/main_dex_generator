...
public class Simulator {
     ...
     */
    private static com.droid.dx.rop.type.Type requiredArrayTypeFor(com.droid.dx.rop.type.Type impliedType,
                                                                   com.droid.dx.rop.type.Type foundArrayType) {
        if (foundArrayType == com.droid.dx.rop.type.Type.KNOWN_NULL) {
            return impliedType.isReference()
                ? com.droid.dx.rop.type.Type.KNOWN_NULL
                : impliedType.getArrayType();
        }

        if ((impliedType == com.droid.dx.rop.type.Type.OBJECT)
                && foundArrayType.isArray()
                && foundArrayType.getComponentType().isReference()) {
            return foundArrayType;
        }

        if ((impliedType == com.droid.dx.rop.type.Type.BYTE)
                && (foundArrayType == com.droid.dx.rop.type.Type.BOOLEAN_ARRAY)) {
            /*
             * Per above, an instruction with implied byte[] is also
             * allowed to be used on boolean[].
             */
            return com.droid.dx.rop.type.Type.BOOLEAN_ARRAY;
        }
    ...
    private class SimVisitor implements BytecodeArray.Visitor {
        ...
        public void visitInvalid(int opcode, int offset, int length) {
            throw new SimException("invalid opcode " + com.droid.dx.util.Hex.u1(opcode));
        }
        ...
        public void visitNoArgs(int opcode, int offset, int length,
            ...
            switch (opcode) {
                ...
                case ByteOps.I2C:
                case ByteOps.I2S: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.INT);
                    break;
                }
                case ByteOps.L2I:
                case ByteOps.L2F:
                case ByteOps.L2D: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.LONG);
                    break;
                }
                case ByteOps.F2I:
                case ByteOps.F2L:
                case ByteOps.F2D: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.FLOAT);
                    break;
                }
                case ByteOps.D2I:
                case ByteOps.D2L:
                case ByteOps.D2F: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.DOUBLE);
                    break;
                }
                case ByteOps.RETURN: {
                    machine.clearArgs();
                    checkReturnType(com.droid.dx.rop.type.Type.VOID);
                    break;
                }
                case ByteOps.IRETURN: {
                    com.droid.dx.rop.type.Type checkType = type;
                    if (type == com.droid.dx.rop.type.Type.OBJECT) {
                        /*
                ...
                case ByteOps.POP: {
                    com.droid.dx.rop.type.Type peekType = frame.getStack().peekType(0);
                ...
                case ByteOps.ARRAYLENGTH: {
                    com.droid.dx.rop.type.Type arrayType = frame.getStack().peekType(0);
                    if (!arrayType.isArrayOrKnownNull()) {
                        throw new SimException("type mismatch: expected " +
                                "array type but encountered " +
                                arrayType.toHuman());
                    }
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.OBJECT);
                    break;
                }
                case ByteOps.ATHROW:
                ...
                case ByteOps.IALOAD: {
                     ...
                     */
                    com.droid.dx.rop.type.Type foundArrayType = frame.getStack().peekType(1);
                    com.droid.dx.rop.type.Type requiredArrayType =
                        requiredArrayTypeFor(type, foundArrayType);

                    // Make type agree with the discovered requiredArrayType.
                    type = (requiredArrayType == com.droid.dx.rop.type.Type.KNOWN_NULL)
                        ? com.droid.dx.rop.type.Type.KNOWN_NULL
                        : requiredArrayType.getComponentType();

                    machine.popArgs(frame, requiredArrayType, com.droid.dx.rop.type.Type.INT);
                    break;
                }
                case ByteOps.IADD:
                case ByteOps.ISUB:
                ...
                case ByteOps.ISHR:
                case ByteOps.IUSHR: {
                    machine.popArgs(frame, type, com.droid.dx.rop.type.Type.INT);
                    break;
                }
                case ByteOps.LCMP: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.LONG, com.droid.dx.rop.type.Type.LONG);
                    break;
                }
                case ByteOps.FCMPL:
                case ByteOps.FCMPG: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.FLOAT, com.droid.dx.rop.type.Type.FLOAT);
                    break;
                }
                case ByteOps.DCMPL:
                case ByteOps.DCMPG: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.DOUBLE, com.droid.dx.rop.type.Type.DOUBLE);
                    break;
                }
                case ByteOps.IASTORE: {
                    ...
                    com.droid.dx.rop.type.Type foundArrayType = stack.peekType(peekDepth);
                    ...
                    machine.popArgs(frame, requiredArrayType, com.droid.dx.rop.type.Type.INT, type);
                    break;
                }
                case ByteOps.POP2:
                case ByteOps.DUP2: {
         ...
         */
        private void checkReturnType(com.droid.dx.rop.type.Type encountered) {
            com.droid.dx.rop.type.Type returnType = machine.getPrototype().getReturnType();
        ...
        public void visitLocal(int opcode, int offset, int length,
            ...
            com.droid.dx.rop.type.Type localType;
            ...
            switch (opcode) {
                ...
                case ByteOps.ISTORE: {
                    com.droid.dx.rop.code.LocalItem item
                ...
                case ByteOps.IINC: {
                    ...
                    machine.auxCstArg(com.droid.dx.rop.cst.CstInteger.make(value));
                    break;
                }
                default: {
                    visitInvalid(opcode, offset, length);
        ...
        public void visitConstant(int opcode, int offset, int length,
            ...
            switch (opcode) {
                ...
                case ByteOps.PUTSTATIC: {
                    com.droid.dx.rop.type.Type fieldType = ((com.droid.dx.rop.cst.CstFieldRef) cst).getType();
                ...
                case ByteOps.PUTFIELD: {
                    com.droid.dx.rop.type.Type fieldType = ((com.droid.dx.rop.cst.CstFieldRef) cst).getType();
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.OBJECT, fieldType);
                    break;
                }
                case ByteOps.INVOKEINTERFACE: {
                     ...
                     * method ref.
                     */
                    cst = ((com.droid.dx.rop.cst.CstInterfaceMethodRef) cst).toMethodRef();
                    // and fall through...
                }
                ...
                case ByteOps.INVOKESPECIAL: {
                     ...
                     */
                    Prototype prototype =
                        ((com.droid.dx.rop.cst.CstMethodRef) cst).getPrototype(false);
                ...
                case ByteOps.INVOKESTATIC: {
                     ...
                     */
                    Prototype prototype =
                        ((com.droid.dx.rop.cst.CstMethodRef) cst).getPrototype(true);
                    machine.popArgs(frame, prototype);
                    break;
                }
                case ByteOps.MULTIANEWARRAY: {
                     ...
                     */
                    Prototype prototype =
                        Prototype.internInts(com.droid.dx.rop.type.Type.VOID, value);
                    machine.popArgs(frame, prototype);
                    break;
                }
                default: {
        ...
        public void visitBranch(int opcode, int offset, int length,
            ...
            switch (opcode) {
                ...
                case ByteOps.IF_ICMPLE: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.INT, com.droid.dx.rop.type.Type.INT);
                    break;
                }
                case ByteOps.IF_ACMPEQ:
                case ByteOps.IF_ACMPNE: {
                    machine.popArgs(frame, com.droid.dx.rop.type.Type.OBJECT, com.droid.dx.rop.type.Type.OBJECT);
                    break;
                }
                case ByteOps.GOTO:
                case ByteOps.JSR:
