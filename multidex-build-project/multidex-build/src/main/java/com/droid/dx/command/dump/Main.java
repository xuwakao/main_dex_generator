...
import com.droid.dx.util.HexParser;
...
public class Main {
    ...
    private static void processOne(String name, byte[] bytes) {
        if (parsedArgs.dotDump) {
            com.droid.dx.command.dump.DotDumper.dump(bytes, name, parsedArgs);
        } else if (parsedArgs.basicBlocks) {
            com.droid.dx.command.dump.BlockDumper.dump(bytes, System.out, name, false, parsedArgs);
        } else if (parsedArgs.ropBlocks) {
            com.droid.dx.command.dump.BlockDumper.dump(bytes, System.out, name, true, parsedArgs);
        } else if (parsedArgs.ssaBlocks) {
            // --optimize ignored with --ssa-blocks
            parsedArgs.optimize = false;
            com.droid.dx.command.dump.SsaDumper.dump(bytes, System.out, name, parsedArgs);
        } else {
            com.droid.dx.command.dump.ClassDumper.dump(bytes, System.out, name, parsedArgs);
        }
    }
}
