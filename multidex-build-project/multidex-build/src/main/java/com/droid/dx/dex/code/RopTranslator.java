...
import com.droid.dx.rop.code.BasicBlockList;
...
import com.droid.dx.rop.code.RegisterSpecSet;
...
/**
 * Translator from {@link com.droid.dx.rop.code.RopMethod} to {@link DalvCode}. The {@link
...
public final class RopTranslator {
    ...
    private final com.droid.dx.rop.code.RopMethod method;
    ...
    private final com.droid.dx.rop.code.LocalVariableInfo locals;
    ...
    /**
     * Translates a {@link com.droid.dx.rop.code.RopMethod}. This may modify the given
     * input.
     *
     ...
     */
    public static DalvCode translate(com.droid.dx.rop.code.RopMethod method, int positionInfo,
                                     com.droid.dx.rop.code.LocalVariableInfo locals, int paramSize, DexOptions dexOptions) {
     ...
     * @param dexOptions {@code non-null;} options for dex output
     */
    private RopTranslator(com.droid.dx.rop.code.RopMethod method, int positionInfo, com.droid.dx.rop.code.LocalVariableInfo locals,
     ...
     * to this method
     */
    private static boolean calculateParamsAreInOrder(com.droid.dx.rop.code.RopMethod method,
                                                     final int paramSize) {
        ...
        method.getBlocks().forEachInsn(new Insn.BaseVisitor() {
            @Override
            public void visitPlainCstInsn(com.droid.dx.rop.code.PlainCstInsn insn) {
                if (insn.getOpcode().getOpcode()== com.droid.dx.rop.code.RegOps.MOVE_PARAM) {
                    int param =
                        ((com.droid.dx.rop.cst.CstInteger) insn.getConstant()).getValue();
    ...
    private DalvCode translateAndGetResult() {
        ...
        com.droid.dx.dex.code.StdCatchBuilder catches =
            new com.droid.dx.dex.code.StdCatchBuilder(method, order, addresses);
    ...
    private void pickOrder() {
        ...
        int[] tracebackSet = com.droid.dx.util.Bits.makeBitSet(maxLabel);

        for (int i = 0; i < sz; i++) {
            BasicBlock one = blocks.get(i);
            com.droid.dx.util.Bits.set(workSet, one.getLabel());
        }
        ...
        for (int label = method.getFirstLabel();
             label != -1;
             label = com.droid.dx.util.Bits.findFirst(workSet, 0)) {
            ...
            for (;;) {
                ...
                for (int i = 0; i < psz; i++) {
                    ...
                    if (com.droid.dx.util.Bits.get(tracebackSet, predLabel)) {
                        /*
                    ...
                    if (!com.droid.dx.util.Bits.get(workSet, predLabel)) {
                    ...
                    if (pred.getPrimarySuccessor() == label) {
                        // Found one!
                        label = predLabel;
                        com.droid.dx.util.Bits.set(tracebackSet, label);
                        continue traceBack;
                    }
                }
             ...
             */
            while (label != -1) {
                com.droid.dx.util.Bits.clear(workSet, label);
                com.droid.dx.util.Bits.clear(tracebackSet, label);
                order[at] = label;
                at++;
                ...
                if (com.droid.dx.util.Bits.get(workSet, preferred)) {
                    /*
                    ...
                    label = preferred;
                        ...
                        && com.droid.dx.util.Bits.get(workSet, primary)) {
                    /*
                    ...
                    for (int i = 0; i < ssz; i++) {
                        ...
                        if (com.droid.dx.util.Bits.get(workSet, candidate)) {
                            label = candidate;
                            break;
                        }
                    }
    ...
    private class TranslationVisitor implements Insn.Visitor {
        ...
        public void visitPlainInsn(PlainInsn insn) {
            ...
            if (rop.getOpcode() == com.droid.dx.rop.code.RegOps.MARK_LOCAL) {
                /*
            ...
            if (rop.getOpcode() == com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
                // These get skipped
                return;
            }
            ...
            com.droid.dx.dex.code.DalvInsn di;

            switch (rop.getBranchingness()) {
                case Rop.BRANCH_NONE:
                case Rop.BRANCH_RETURN:
                case Rop.BRANCH_THROW: {
                    di = new com.droid.dx.dex.code.SimpleInsn(opcode, pos, getRegs(insn));
                    break;
                }
        ...
        public void visitPlainCstInsn(com.droid.dx.rop.code.PlainCstInsn insn) {
            ...
            if (ropOpcode == com.droid.dx.rop.code.RegOps.MOVE_PARAM) {
                if (!paramsAreInOrder) {
                    /*
                    ...
                    RegisterSpec source =
                        RegisterSpec.make(regCount - paramSize + param,
                                dest.getType());
                    di = new com.droid.dx.dex.code.SimpleInsn(opcode, pos,
                ...
                di = new com.droid.dx.dex.code.CstInsn(opcode, pos, regs, insn.getConstant());
                addOutput(di);
            }
        }

        /** {@inheritDoc} */
        public void visitSwitchInsn(com.droid.dx.rop.code.SwitchInsn insn) {
            SourcePosition pos = insn.getPosition();
            ...
            addOutputSuffix(new com.droid.dx.dex.code.OddSpacer(pos));
        ...
        private RegisterSpec getNextMoveResultPseudo()
        {
            ...
            if (insn.getOpcode().getOpcode() != com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
                return null;
            } else {
        ...
        /** {@inheritDoc} */
        public void visitThrowingCstInsn(com.droid.dx.rop.code.ThrowingCstInsn insn) {
            SourcePosition pos = insn.getPosition();
            Dop opcode = RopToDop.dopFor(insn);
            Rop rop = insn.getOpcode();
            ...
            if (rop.isCallLike()) {
                RegisterSpecList regs = insn.getSources();
                com.droid.dx.dex.code.DalvInsn di = new com.droid.dx.dex.code.CstInsn(opcode, pos, regs, cst);
                ...
                boolean hasResult = opcode.hasResult()
                        || (rop.getOpcode() == com.droid.dx.rop.code.RegOps.CHECK_CAST);
                ...
                if ((rop.getOpcode() == com.droid.dx.rop.code.RegOps.NEW_ARRAY) &&
                    (opcode.getOpcode() != com.droid.dx.io.Opcodes.NEW_ARRAY)) {
                    /*
                     * It's a type-specific new-array-<primitive>, and
                     * so it should be turned into a SimpleInsn (no
                     * constant ref as it's implicit).
                     */
                    di = new com.droid.dx.dex.code.SimpleInsn(opcode, pos, regs);
                } else {
                    /*
                     * This is the general case for constant-bearing
                     * instructions.
                     */
                    di = new com.droid.dx.dex.code.CstInsn(opcode, pos, regs, cst);
                }
        ...
        /** {@inheritDoc} */
        public void visitThrowingInsn(com.droid.dx.rop.code.ThrowingInsn insn) {
            SourcePosition pos = insn.getPosition();
            Dop opcode = RopToDop.dopFor(insn);
            Rop rop = insn.getOpcode();
            ...
            com.droid.dx.dex.code.DalvInsn di = new com.droid.dx.dex.code.SimpleInsn(opcode, pos,
        ...
        public void visitFillArrayDataInsn(FillArrayDataInsn insn) {
            ...
            CodeAddress dataAddress = new CodeAddress(pos);
            com.droid.dx.dex.code.ArrayData dataInsn =
                new com.droid.dx.dex.code.ArrayData(pos, lastAddress, values, cst);
         ...
         */
        protected void addOutput(com.droid.dx.dex.code.DalvInsn insn) {
            output.add(insn);
        }
         ...
         * @param insn {@code non-null;} instruction to add
         */
        protected void addOutputSuffix(com.droid.dx.dex.code.DalvInsn insn) {
    ...
    private class LocalVariableAwareTranslationVisitor
        ...
        private com.droid.dx.rop.code.LocalVariableInfo locals;
        ...
        public LocalVariableAwareTranslationVisitor(OutputCollector output,
                                                    com.droid.dx.rop.code.LocalVariableInfo locals) {
            super(output);
            this.locals = locals;
        }
