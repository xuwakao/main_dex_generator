...
package com.droid.dx.command.annotool;

import com.droid.dx.cf.attrib.BaseAnnotations;
...
class AnnotationLister {
    ...
    private final com.droid.dx.command.annotool.Main.Arguments args;
    ...
    AnnotationLister (com.droid.dx.command.annotool.Main.Arguments args) {
        this.args = args;
    }
        ...
        for (String path : args.files) {
            ...
            opener = new ClassPathOpener(path, true,
                ...
                public boolean processFileBytes(String name, long lastModified, byte[] bytes) {
                    ...
                    com.droid.dx.cf.direct.DirectClassFile cf
                        = new com.droid.dx.cf.direct.DirectClassFile(ba, name, true);

                    cf.setAttributeFactory(com.droid.dx.cf.direct.StdAttributeFactory.THE_ONE);
                    com.droid.dx.cf.iface.AttributeList attributes = cf.getAttributes();
                    Attribute att;
                    ...
                    if (cfClassName.endsWith(PACKAGE_INFO)) {
                        att = attributes.findFirst(
                                com.droid.dx.cf.attrib.AttRuntimeInvisibleAnnotations.ATTRIBUTE_NAME);
     ...
     */
    private void visitClassAnnotation(com.droid.dx.cf.direct.DirectClassFile cf,
                                      BaseAnnotations ann) {
    ...
    private void visitPackageAnnotation(
            com.droid.dx.cf.direct.DirectClassFile cf, BaseAnnotations ann) {
    ...
    private void printMatchPackage(String packageName) {
        for (com.droid.dx.command.annotool.Main.PrintType pt : args.printTypes) {
            switch (pt) {
                case CLASS:
                case INNERCLASS:
                case METHOD:
     ...
     */
    private void printMatch(com.droid.dx.cf.direct.DirectClassFile cf) {
        for (com.droid.dx.command.annotool.Main.PrintType pt : args.printTypes) {
            switch (pt) {
                case CLASS:
                    String classname;
