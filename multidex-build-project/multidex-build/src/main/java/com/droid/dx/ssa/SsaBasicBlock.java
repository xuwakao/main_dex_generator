...
import com.droid.dx.rop.code.InsnList;
...
import com.droid.dx.util.IntSet;
...
public final class SsaBasicBlock {
    ...
    private ArrayList<com.droid.dx.ssa.SsaInsn> insns;
    ...
    private com.droid.dx.ssa.SsaMethod parent;
    ...
    public SsaBasicBlock(final int basicBlockIndex, final int ropLabel,
            final com.droid.dx.ssa.SsaMethod parent) {
        this.parent = parent;
        this.index = basicBlockIndex;
        this.insns = new ArrayList<com.droid.dx.ssa.SsaInsn>();
    ...
    public static SsaBasicBlock newFromRop(RopMethod rmeth,
            int basicBlockIndex, final com.droid.dx.ssa.SsaMethod parent) {
        ...
        for (int i = 0, sz = ropInsns.size() ; i < sz ; i++) {
            result.insns.add(new com.droid.dx.ssa.NormalSsaInsn(ropInsns.get(i), result));
        }

        result.predecessors = com.droid.dx.ssa.SsaMethod.bitSetFromLabelList(
                ropBlocks,
                rmeth.labelToPredecessors(bb.getLabel()));

        result.successors
                = com.droid.dx.ssa.SsaMethod.bitSetFromLabelList(ropBlocks, bb.getSuccessors());

        result.successorList
                = com.droid.dx.ssa.SsaMethod.indexListFromLabelList(ropBlocks,
                    bb.getSuccessors());
    ...
    public void addInsnToHead(Insn insn) {
        com.droid.dx.ssa.SsaInsn newInsn = com.droid.dx.ssa.SsaInsn.makeFromRop(insn, this);
    ...
    public void replaceLastInsn(Insn insn) {
        ...
        com.droid.dx.ssa.SsaInsn oldInsn = insns.get(insns.size() - 1);
        com.droid.dx.ssa.SsaInsn newInsn = com.droid.dx.ssa.SsaInsn.makeFromRop(insn, this);
    ...
    private int getCountPhiInsns() {
        ...
        for (countPhiInsns = 0; countPhiInsns < sz; countPhiInsns++) {
            com.droid.dx.ssa.SsaInsn insn = insns.get(countPhiInsns);
            if (!(insn instanceof PhiInsn)) {
                break;
            }
        }
     ...
     */
    public ArrayList<com.droid.dx.ssa.SsaInsn> getInsns() {
        return insns;
    }
     ...
     */
    public List<com.droid.dx.ssa.SsaInsn> getPhiInsns() {
     ...
     */
    public com.droid.dx.ssa.SsaMethod getParent() {
        return parent;
    }
     ...
     * nothing. For use by {@link com.droid.dx.ssa.SsaMethod#makeExitBlock}
     *
    ...
    public void addMoveToEnd(RegisterSpec result, RegisterSpec source) {
         ...
         */
        com.droid.dx.ssa.NormalSsaInsn lastInsn;
        lastInsn = (com.droid.dx.ssa.NormalSsaInsn)insns.get(insns.size()-1);
        ...
        if (lastInsn.getResult() != null || lastInsn.getSources().size() > 0) {
            ...
            com.droid.dx.ssa.NormalSsaInsn toAdd = new com.droid.dx.ssa.NormalSsaInsn(
     ...
     */
    private void scheduleUseBeforeAssigned(List<com.droid.dx.ssa.SsaInsn> toSchedule) {
        ...
        while (insertPlace < sz) {
             ...
             */
            for (int i = insertPlace; i <sz; i++) {
                com.droid.dx.ssa.SsaInsn insn = toSchedule.get(i);
            ...
            if (oldInsertPlace == insertPlace) {
                ...
                com.droid.dx.ssa.SsaInsn insnToSplit = null;
                ...
                com.droid.dx.ssa.NormalSsaInsn toReplace = new com.droid.dx.ssa.NormalSsaInsn(
                        new PlainInsn(Rops.opMove(result.getType()),
                                SourcePosition.NO_INFO,
                                result,
                                newSources), this);
    ...
    public void addLiveOut (int regV) {
        if (liveOut == null) {
            liveOut = com.droid.dx.ssa.SetFactory.makeLivenessSet(parent.getRegCount());
        }
    ...
    public void addLiveIn (int regV) {
        if (liveIn == null) {
            liveIn = com.droid.dx.ssa.SetFactory.makeLivenessSet(parent.getRegCount());
        }
    ...
    public void scheduleMovesFromPhis() {
        if (movesFromPhisAtBeginning > 1) {
            List<com.droid.dx.ssa.SsaInsn> toSchedule;

            toSchedule = insns.subList(0, movesFromPhisAtBeginning);

            scheduleUseBeforeAssigned(toSchedule);

            com.droid.dx.ssa.SsaInsn firstNonPhiMoveInsn = insns.get(movesFromPhisAtBeginning);
            ...
            if (firstNonPhiMoveInsn.isMoveException()) {
                if (true) {
                     ...
                     * phi moves?
                     */
                    for (com.droid.dx.ssa.SsaInsn insn : toSchedule) {
