...
public class AttributeFactory {
    ...
    public final Attribute parse(DirectClassFile cf, int context, int offset,
        ...
        com.droid.dx.rop.cst.CstString name = null;

        try {
            com.droid.dx.util.ByteArray bytes = cf.getBytes();
            com.droid.dx.rop.cst.ConstantPool pool = cf.getConstantPool();
            int nameIdx = bytes.getUnsignedShort(offset);
            int length = bytes.getInt(offset + 2);

            name = (com.droid.dx.rop.cst.CstString) pool.get(nameIdx);
            ...
            return parse0(cf, context, name.getString(), offset + 6, length,
                          observer);
        } catch (com.droid.dx.cf.iface.ParseException ex) {
     ...
     * an instance of {@link com.droid.dx.cf.attrib.RawAttribute}. Subclasses are expected to
    ...
    protected Attribute parse0(DirectClassFile cf, int context, String name,
        ...
        com.droid.dx.util.ByteArray bytes = cf.getBytes();
        com.droid.dx.rop.cst.ConstantPool pool = cf.getConstantPool();
        Attribute result = new com.droid.dx.cf.attrib.RawAttribute(name, bytes, offset, length, pool);
