...
public final class SsaMethod {
     ...
     */
    private com.droid.dx.ssa.SsaInsn[] definitionList;

    /** indexed by register: the list of all insns that use a register */
    private ArrayList<com.droid.dx.ssa.SsaInsn>[] useList;

    /** A version of useList with each List unmodifiable */
    private List<com.droid.dx.ssa.SsaInsn>[] unmodifiableUseList;
     ...
     * pointer argument
     */
    public static SsaMethod newFromRopMethod(com.droid.dx.rop.code.RopMethod ropMethod,
     ...
     * pointer argument
     */
    private SsaMethod(com.droid.dx.rop.code.RopMethod ropMethod, int paramWidth, boolean isStatic) {
    ...
    private void convertRopToSsaBlocks(com.droid.dx.rop.code.RopMethod rmeth) {
     ...
     */
    private static com.droid.dx.ssa.SsaInsn getGoto(SsaBasicBlock block) {
        return new com.droid.dx.ssa.NormalSsaInsn(
     ...
     */
    public com.droid.dx.ssa.SsaInsn getDefinitionForRegister(int reg) {
        if (backMode) {
        ...
        definitionList = new com.droid.dx.ssa.SsaInsn[getRegCount()];

        forEachInsn(new com.droid.dx.ssa.SsaInsn.Visitor() {
            public void visitMoveInsn (com.droid.dx.ssa.NormalSsaInsn insn) {
                definitionList[insn.getResult().getReg()] = insn;
            }
            public void visitPhiInsn (com.droid.dx.ssa.PhiInsn phi) {
    ...
    private void buildUseList() {
        ...
        forEachInsn(new com.droid.dx.ssa.SsaInsn.Visitor() {
             ...
             */
            private void addToUses(com.droid.dx.ssa.SsaInsn insn) {
     ...
     */
    /*package*/ void onSourceChanged(com.droid.dx.ssa.SsaInsn insn,
     ...
     * previously used
     */
    /*package*/ void onSourcesChanged(com.droid.dx.ssa.SsaInsn insn,
     ...
     */
    private void removeFromUseList(com.droid.dx.ssa.SsaInsn insn, RegisterSpecList oldSources) {
        if (oldSources == null) {
            return;
        }
     ...
     */
    /*package*/ void onInsnAdded(com.droid.dx.ssa.SsaInsn insn) {
     ...
     */
    /*package*/ void onInsnRemoved(com.droid.dx.ssa.SsaInsn insn) {
        if (useList != null) {
     ...
     */
    /*package*/ void updateOneDefinition(com.droid.dx.ssa.SsaInsn insn,
     ...
     */
    public List<com.droid.dx.ssa.SsaInsn> getUseListForRegister(int reg) {
     ...
     */
    public ArrayList<com.droid.dx.ssa.SsaInsn>[] getUseListCopy() {
        if (useList == null) {
            buildUseList();
        }

        ArrayList<com.droid.dx.ssa.SsaInsn>[] useListCopy
                = (ArrayList<com.droid.dx.ssa.SsaInsn>[])(new ArrayList[registerCount]);

        for (int i = 0; i < registerCount; i++) {
            useListCopy[i] = (ArrayList<com.droid.dx.ssa.SsaInsn>)(new ArrayList(useList[i]));
        }
    ...
    public boolean isRegALocal(RegisterSpec spec) {
        com.droid.dx.ssa.SsaInsn defn = getDefinitionForRegister(spec.getReg());
        ...
        for (com.droid.dx.ssa.SsaInsn use : getUseListForRegister(spec.getReg())) {
            Insn insn = use.getOriginalRopInsn();

            if (insn != null
                    && insn.getOpcode().getOpcode() == com.droid.dx.rop.code.RegOps.MARK_LOCAL) {
                return true;
            }
        }
     ...
     */
    public void forEachInsn(com.droid.dx.ssa.SsaInsn.Visitor visitor) {
        for (SsaBasicBlock block : blocks) {
     ...
     *
     */
    public void forEachPhiInsn(com.droid.dx.ssa.PhiInsn.Visitor v) {
        for (SsaBasicBlock block : blocks) {
     ...
     */
    public void deleteInsns(Set<com.droid.dx.ssa.SsaInsn> deletedInsns) {
        for (SsaBasicBlock block : getBlocks()) {
            ArrayList<com.droid.dx.ssa.SsaInsn> insns = block.getInsns();

            for (int i = insns.size() - 1; i >= 0; i--) {
                com.droid.dx.ssa.SsaInsn insn = insns.get(i);
            ...
            com.droid.dx.ssa.SsaInsn lastInsn = (insnsSz == 0) ? null : insns.get(insnsSz - 1);
            ...
            if (block != getExitBlock() && (insnsSz == 0
                ...
                insns.add(com.droid.dx.ssa.SsaInsn.makeFromRop(gotoInsn, block));
