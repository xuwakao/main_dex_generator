...
public final class Ropper {
    ...
    private class CatchInfo {
        ...
        private final Map<com.droid.dx.rop.type.Type, ExceptionHandlerSetup> setups =
                new HashMap<com.droid.dx.rop.type.Type, ExceptionHandlerSetup>();
         ...
         */
        ExceptionHandlerSetup getSetup(com.droid.dx.rop.type.Type caughtType) {
    ...
    private static class ExceptionHandlerSetup {
        ...
        private com.droid.dx.rop.type.Type caughtType;
        /**
         ...
         */
        ExceptionHandlerSetup(com.droid.dx.rop.type.Type caughtType, int label) {
         ...
         */
        com.droid.dx.rop.type.Type getCaughtType() {
            return caughtType;
        }
        ...
        void mergeToSuccessors(Frame frame, int[] workSet) {
            for (int label = callerBlocks.nextSetBit(0); label >= 0;
                ...
                if (subFrame != null) {
                    ...
                    com.droid.dx.util.Bits.set(workSet, label);
                }
            }
        }
    }
    ...
    /**
     * Converts a {@link ConcreteMethod} to a {@link com.droid.dx.rop.code.RopMethod}.
     *
     ...
     */
    public static com.droid.dx.rop.code.RopMethod convert(ConcreteMethod method,
                                                          com.droid.dx.rop.code.TranslationAdvice advice, com.droid.dx.cf.iface.MethodList methods) {
        try {
     ...
     *     that defines {@code method}.
     */
    private Ropper(ConcreteMethod method, com.droid.dx.rop.code.TranslationAdvice advice, com.droid.dx.cf.iface.MethodList methods) {
        if (method == null) {
    ...
    private boolean isSynchronized() {
        ...
        return (accessFlags & com.droid.dx.rop.code.AccessFlags.ACC_SYNCHRONIZED) != 0;
    }
    ...
    private boolean isStatic() {
        int accessFlags = method.getAccessFlags();
        return (accessFlags & com.droid.dx.rop.code.AccessFlags.ACC_STATIC) != 0;
    }
     ...
     */
    private com.droid.dx.rop.code.RegisterSpec getSynchReg() {
        /*
         * We use the register that is just past the deepest possible
        ...
        return com.droid.dx.rop.code.RegisterSpec.make((reg < 1) ? 1 : reg, com.droid.dx.rop.type.Type.OBJECT);
    }
    ...
    /**
     * Extracts the resulting {@link com.droid.dx.rop.code.RopMethod} from the instance.
     *
     * @return {@code non-null;} the method object
     */
    private com.droid.dx.rop.code.RopMethod getRopMethod() {

        // Construct the final list of blocks.

        int sz = result.size();
        com.droid.dx.rop.code.BasicBlockList bbl = new com.droid.dx.rop.code.BasicBlockList(sz);
        for (int i = 0; i < sz; i++) {
         ...
         * constructor.
         */
        return new com.droid.dx.rop.code.RopMethod(bbl, getSpecialLabel(PARAM_ASSIGNMENT));
    }
     ...
     */
    private void doit() {
        int[] workSet = com.droid.dx.util.Bits.makeBitSet(maxLabel);

        com.droid.dx.util.Bits.set(workSet, 0);
        addSetupBlocks();
        setFirstFrame();

        for (;;) {
            int offset = com.droid.dx.util.Bits.findFirst(workSet, 0);
            if (offset < 0) {
                break;
            }
            com.droid.dx.util.Bits.clear(workSet, offset);
    ...
    private void processBlock(ByteBlock block, Frame frame, int[] workSet) {
        ...
        com.droid.dx.cf.code.ByteCatchList catches = block.getCatches();
        ...
        ArrayList<com.droid.dx.rop.code.Insn> insns = machine.getInsns();
        ...
        if (synch || (catchSz != 0)) {
            ...
            for (int i = 0; i < catchSz; i++) {
                com.droid.dx.cf.code.ByteCatchList.Item one = catches.get(i);
                com.droid.dx.rop.cst.CstType exceptionClass = one.getExceptionClass();
                int targ = one.getHandlerPc();

                catchesAny |= (exceptionClass == com.droid.dx.rop.cst.CstType.OBJECT);
            ...
            if (synch && !catchesAny) {
                ...
                for (int i = insnSz - extraBlockCount - 1; i < insnSz; i++) {
                    com.droid.dx.rop.code.Insn insn = insns.get(i);
                    if (insn.canThrow()) {
                        insn = insn.withAddedCatch(com.droid.dx.rop.type.Type.OBJECT);
                        insns.set(i, insn);
                    }
                }
            }
        ...
        for (/*extraBlockCount*/; extraBlockCount > 0; extraBlockCount--) {
             ...
             */
            com.droid.dx.rop.code.Insn extraInsn = insns.get(--insnSz);
            boolean needsGoto
                    = extraInsn.getOpcode().getBranchingness()
                        == Rop.BRANCH_NONE;
            com.droid.dx.rop.code.InsnList il = new com.droid.dx.rop.code.InsnList(needsGoto ? 2 : 1);
            IntList extraBlockSuccessors = successors;

            il.set(0, extraInsn);

            if (needsGoto) {
                il.set(1, new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.GOTO,
                        extraInsn.getPosition(), null,
                        com.droid.dx.rop.code.RegisterSpecList.EMPTY));
                /*
        ...
        com.droid.dx.rop.code.Insn lastInsn = (insnSz == 0) ? null : insns.get(insnSz - 1);
        ...
        if ((lastInsn == null) ||
            ...
            insns.add(new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.GOTO, pos, null,
                                    com.droid.dx.rop.code.RegisterSpecList.EMPTY));
            insnSz++;
        }
        ...
        com.droid.dx.rop.code.InsnList il = new com.droid.dx.rop.code.InsnList(insnSz);
    ...
    private void addSetupBlocks() {
        ...
        Prototype desc = method.getEffectiveDescriptor();
        com.droid.dx.rop.type.StdTypeList params = desc.getParameterTypes();
        int sz = params.size();
        com.droid.dx.rop.code.InsnList insns = new com.droid.dx.rop.code.InsnList(sz + 1);
        int at = 0;

        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.type.Type one = params.get(i);
            LocalVariableList.Item local =
                localVariables.pcAndIndexToLocal(0, at);
            com.droid.dx.rop.code.RegisterSpec result = (local == null) ?
                com.droid.dx.rop.code.RegisterSpec.make(at, one) :
                com.droid.dx.rop.code.RegisterSpec.makeLocalOptional(at, one, local.getLocalItem());

            com.droid.dx.rop.code.Insn insn = new com.droid.dx.rop.code.PlainCstInsn(com.droid.dx.rop.code.Rops.opMoveParam(one), pos, result,
                                         com.droid.dx.rop.code.RegisterSpecList.EMPTY,
                                         com.droid.dx.rop.cst.CstInteger.make(at));
            insns.set(i, insn);
            at += one.getCategory();
        }

        insns.set(sz, new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.GOTO, pos, null,
                                    com.droid.dx.rop.code.RegisterSpecList.EMPTY));
        insns.setImmutable();
        ...
        if (synch) {
            com.droid.dx.rop.code.RegisterSpec synchReg = getSynchReg();
            com.droid.dx.rop.code.Insn insn;
            if (isStatic()) {
                insn = new com.droid.dx.rop.code.ThrowingCstInsn(com.droid.dx.rop.code.Rops.CONST_OBJECT, pos,
                                           com.droid.dx.rop.code.RegisterSpecList.EMPTY,
                                           com.droid.dx.rop.type.StdTypeList.EMPTY,
                                           method.getDefiningClass());
                insns = new com.droid.dx.rop.code.InsnList(1);
                insns.set(0, insn);
            } else {
                insns = new com.droid.dx.rop.code.InsnList(2);
                insn = new com.droid.dx.rop.code.PlainCstInsn(com.droid.dx.rop.code.Rops.MOVE_PARAM_OBJECT, pos,
                                        synchReg, com.droid.dx.rop.code.RegisterSpecList.EMPTY,
                                        com.droid.dx.rop.cst.CstInteger.VALUE_0);
                insns.set(0, insn);
                insns.set(1, new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.GOTO, pos, null,
                                           com.droid.dx.rop.code.RegisterSpecList.EMPTY));
            }
            ...
            insns = new com.droid.dx.rop.code.InsnList(isStatic() ? 2 : 1);

            if (isStatic()) {
                insns.set(0, new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.opMoveResultPseudo(synchReg),
                        pos, synchReg, com.droid.dx.rop.code.RegisterSpecList.EMPTY));
            }

            insn = new com.droid.dx.rop.code.ThrowingInsn(com.droid.dx.rop.code.Rops.MONITOR_ENTER, pos,
                                    com.droid.dx.rop.code.RegisterSpecList.make(synchReg),
                                    com.droid.dx.rop.type.StdTypeList.EMPTY);
    ...
    private void addReturnBlock() {
        ...
        if (isSynchronized()) {
            com.droid.dx.rop.code.InsnList insns = new com.droid.dx.rop.code.InsnList(1);
            com.droid.dx.rop.code.Insn insn = new com.droid.dx.rop.code.ThrowingInsn(com.droid.dx.rop.code.Rops.MONITOR_EXIT, returnPos,
                                         com.droid.dx.rop.code.RegisterSpecList.make(getSynchReg()),
                                         com.droid.dx.rop.type.StdTypeList.EMPTY);
            insns.set(0, insn);
            insns.setImmutable();
        ...
        com.droid.dx.rop.code.InsnList insns = new com.droid.dx.rop.code.InsnList(1);
        com.droid.dx.rop.type.TypeList sourceTypes = returnOp.getSources();
        com.droid.dx.rop.code.RegisterSpecList sources;

        if (sourceTypes.size() == 0) {
            sources = com.droid.dx.rop.code.RegisterSpecList.EMPTY;
        } else {
            com.droid.dx.rop.code.RegisterSpec source = com.droid.dx.rop.code.RegisterSpec.make(0, sourceTypes.getType(0));
            sources = com.droid.dx.rop.code.RegisterSpecList.make(source);
        }

        com.droid.dx.rop.code.Insn insn = new com.droid.dx.rop.code.PlainInsn(returnOp, returnPos, null, sources);
        insns.set(0, insn);
        insns.setImmutable();
    ...
    private void addSynchExceptionHandlerBlock() {
        ...
        SourcePosition pos = method.makeSourcePosistion(0);
        com.droid.dx.rop.code.RegisterSpec exReg = com.droid.dx.rop.code.RegisterSpec.make(0, com.droid.dx.rop.type.Type.THROWABLE);
        BasicBlock bb;
        com.droid.dx.rop.code.Insn insn;

        com.droid.dx.rop.code.InsnList insns = new com.droid.dx.rop.code.InsnList(2);
        insn = new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.opMoveException(com.droid.dx.rop.type.Type.THROWABLE), pos,
                             exReg, com.droid.dx.rop.code.RegisterSpecList.EMPTY);
        insns.set(0, insn);
        insn = new com.droid.dx.rop.code.ThrowingInsn(com.droid.dx.rop.code.Rops.MONITOR_EXIT, pos,
                                com.droid.dx.rop.code.RegisterSpecList.make(getSynchReg()),
                                com.droid.dx.rop.type.StdTypeList.EMPTY);
        insns.set(1, insn);
        insns.setImmutable();
        ...
        insns = new com.droid.dx.rop.code.InsnList(1);
        insn = new com.droid.dx.rop.code.ThrowingInsn(com.droid.dx.rop.code.Rops.THROW, pos,
                                com.droid.dx.rop.code.RegisterSpecList.make(exReg),
                                com.droid.dx.rop.type.StdTypeList.EMPTY);
        insns.set(0, insn);
        insns.setImmutable();
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (catches != null) {
                for (ExceptionHandlerSetup one : catches.getSetups()) {
                    com.droid.dx.rop.code.Insn proto = labelToBlock(i).getFirstInsn();
                    SourcePosition pos = proto.getPosition();
                    com.droid.dx.rop.code.InsnList il = new com.droid.dx.rop.code.InsnList(2);

                    com.droid.dx.rop.code.Insn insn = new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.opMoveException(one.getCaughtType()),
                            pos,
                            com.droid.dx.rop.code.RegisterSpec.make(maxLocals, one.getCaughtType()),
                            com.droid.dx.rop.code.RegisterSpecList.EMPTY);
                    il.set(0, insn);

                    insn = new com.droid.dx.rop.code.PlainInsn(com.droid.dx.rop.code.Rops.GOTO, pos, null,
                            com.droid.dx.rop.code.RegisterSpecList.EMPTY);
                    il.set(1, insn);
                    il.setImmutable();
     ...
     */
    private com.droid.dx.rop.code.InsnList filterMoveReturnAddressInsns(com.droid.dx.rop.code.InsnList insns) {
        int sz;
        int newSz = 0;
        ...
        sz = insns.size();
        for (int i = 0; i < sz; i++) {
            if (insns.get(i).getOpcode() != com.droid.dx.rop.code.Rops.MOVE_RETURN_ADDRESS) {
                newSz++;
            }
        }
        ...
        com.droid.dx.rop.code.InsnList newInsns = new com.droid.dx.rop.code.InsnList(newSz);

        int newIndex = 0;
        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.Insn insn = insns.get(i);
            if (insn.getOpcode() != com.droid.dx.rop.code.Rops.MOVE_RETURN_ADDRESS) {
