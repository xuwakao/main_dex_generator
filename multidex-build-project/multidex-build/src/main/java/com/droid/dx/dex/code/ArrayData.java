...
public final class ArrayData extends VariableSizeInsn {
    ...
    public ArrayData(SourcePosition position, CodeAddress user,
        ...
        if (arrayType == com.droid.dx.rop.cst.CstType.BYTE_ARRAY ||
                arrayType == com.droid.dx.rop.cst.CstType.BOOLEAN_ARRAY) {
            elemWidth = 1;
        } else if (arrayType == com.droid.dx.rop.cst.CstType.SHORT_ARRAY ||
                arrayType == com.droid.dx.rop.cst.CstType.CHAR_ARRAY) {
            elemWidth = 2;
        } else if (arrayType == com.droid.dx.rop.cst.CstType.INT_ARRAY ||
                arrayType == com.droid.dx.rop.cst.CstType.FLOAT_ARRAY) {
            elemWidth = 4;
        } else if (arrayType == com.droid.dx.rop.cst.CstType.LONG_ARRAY ||
                arrayType == com.droid.dx.rop.cst.CstType.DOUBLE_ARRAY) {
            elemWidth = 8;
        } else {
    ...
    public void writeTo(AnnotatedOutput out) {
        ...
        out.writeShort(com.droid.dx.io.Opcodes.FILL_ARRAY_DATA_PAYLOAD);
        ...
        switch (elemWidth) {
            case 1: {
                for (int i = 0; i < sz; i++) {
                    ...
                    out.writeByte((byte) ((com.droid.dx.rop.cst.CstLiteral32) cst).getIntBits());
                }
                break;
            }
            case 2: {
                for (int i = 0; i < sz; i++) {
                    Constant cst = values.get(i);
                    out.writeShort((short) ((com.droid.dx.rop.cst.CstLiteral32) cst).getIntBits());
                }
                break;
            }
            case 4: {
                for (int i = 0; i < sz; i++) {
                    Constant cst = values.get(i);
                    out.writeInt(((com.droid.dx.rop.cst.CstLiteral32) cst).getIntBits());
                }
                break;
            }
            case 8: {
                for (int i = 0; i < sz; i++) {
                    Constant cst = values.get(i);
                    out.writeLong(((com.droid.dx.rop.cst.CstLiteral64) cst).getLongBits());
                }
                break;
            }
            default:
    ...
    /** {@inheritDoc} */
    @Override
    public com.droid.dx.dex.code.DalvInsn withRegisters(com.droid.dx.rop.code.RegisterSpecList registers) {
