...
package com.droid.dx.dex.code.form;

import com.droid.dx.dex.code.DalvInsn;
import com.droid.dx.dex.code.InsnFormat;
...
public final class Form35c extends InsnFormat {
    ...
    public String insnArgString(DalvInsn insn) {
        com.droid.dx.rop.code.RegisterSpecList regs = explicitize(insn.getRegisters());
    ...
    public boolean isCompatible(DalvInsn insn) {
        ...
        com.droid.dx.dex.code.CstInsn ci = (com.droid.dx.dex.code.CstInsn) insn;
        int cpi = ci.getIndex();
        ...
        if (!((cst instanceof com.droid.dx.rop.cst.CstMethodRef) ||
              (cst instanceof com.droid.dx.rop.cst.CstType))) {
            return false;
        }

        com.droid.dx.rop.code.RegisterSpecList regs = ci.getRegisters();
    ...
    public BitSet compatibleRegs(DalvInsn insn) {
        com.droid.dx.rop.code.RegisterSpecList regs = insn.getRegisters();
        int sz = regs.size();
        BitSet bits = new BitSet(sz);

        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.RegisterSpec reg = regs.get(i);
            /*
    ...
    public void writeTo(AnnotatedOutput out, DalvInsn insn) {
        int cpi = ((com.droid.dx.dex.code.CstInsn) insn).getIndex();
        com.droid.dx.rop.code.RegisterSpecList regs = explicitize(insn.getRegisters());
        int sz = regs.size();
     ...
     */
    private static int wordCount(com.droid.dx.rop.code.RegisterSpecList regs) {
        int sz = regs.size();
        ...
        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.RegisterSpec one = regs.get(i);
     ...
     */
    private static com.droid.dx.rop.code.RegisterSpecList explicitize(com.droid.dx.rop.code.RegisterSpecList orig) {
        ...
        com.droid.dx.rop.code.RegisterSpecList result = new com.droid.dx.rop.code.RegisterSpecList(wordCount);
        int wordAt = 0;

        for (int i = 0; i < sz; i++) {
            com.droid.dx.rop.code.RegisterSpec one = orig.get(i);
            result.set(wordAt, one);
            if (one.getCategory() == 2) {
                result.set(wordAt + 1,
                           com.droid.dx.rop.code.RegisterSpec.make(one.getReg() + 1, com.droid.dx.rop.type.Type.VOID));
                wordAt += 2;
            } else {
                wordAt++;
            }
