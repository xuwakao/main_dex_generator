...
public class FirstFitLocalCombiningAllocator extends RegisterAllocator {
    ...
    private final Map<com.droid.dx.rop.code.LocalItem, ArrayList<RegisterSpec>> localVariables;

    /** list of move-result-pesudo instructions seen in this method */
    private final ArrayList<com.droid.dx.ssa.NormalSsaInsn> moveResultPseudoInsns;

    /** list of invoke-range instructions seen in this method */
    private final ArrayList<com.droid.dx.ssa.NormalSsaInsn> invokeRangeInsns;

    /** list of phi instructions seen in this method */
    private final ArrayList<com.droid.dx.ssa.PhiInsn> phiInsns;
    ...
    private final com.droid.dx.ssa.InterferenceRegisterMapper mapper;
    ...
    public FirstFitLocalCombiningAllocator(
            com.droid.dx.ssa.SsaMethod ssaMeth, InterferenceGraph interference,
            boolean minimizeRegisters) {
        super(ssaMeth, interference);

        ssaRegsMapped = new BitSet(ssaMeth.getRegCount());

        mapper = new com.droid.dx.ssa.InterferenceRegisterMapper(
        ...
        localVariables = new TreeMap<com.droid.dx.rop.code.LocalItem, ArrayList<RegisterSpec>>();
        moveResultPseudoInsns = new ArrayList<com.droid.dx.ssa.NormalSsaInsn>();
        invokeRangeInsns = new ArrayList<com.droid.dx.ssa.NormalSsaInsn>();
        phiInsns = new ArrayList<com.droid.dx.ssa.PhiInsn>();
    }
    ...
    /** {@inheritDoc} */
    @Override
    public com.droid.dx.ssa.RegisterMapper allocateRegisters() {
    ...
    private void printLocalVars() {
        ...
        for (Map.Entry<com.droid.dx.rop.code.LocalItem, ArrayList<RegisterSpec>> e :
    ...
    private int getParameterIndexForReg(int ssaReg) {
        com.droid.dx.ssa.SsaInsn defInsn = ssaMeth.getDefinitionForRegister(ssaReg);
        if (defInsn == null) {
            return -1;
        }
        ...
        if (opcode != null && opcode.getOpcode() == com.droid.dx.rop.code.RegOps.MOVE_PARAM) {
            CstInsn origInsn = (CstInsn) defInsn.getOriginalRopInsn();
            return  ((com.droid.dx.rop.cst.CstInteger) origInsn.getConstant()).getValue();
        }
    ...
    private void handleInvokeRangeInsns() {
        for (com.droid.dx.ssa.NormalSsaInsn insn : invokeRangeInsns) {
    ...
    private void handleCheckCastResults() {
        for (com.droid.dx.ssa.NormalSsaInsn insn : moveResultPseudoInsns) {
            ...
            com.droid.dx.ssa.SsaBasicBlock predBlock =
                    ssaMeth.getBlocks().get(predBlocks.nextSetBit(0));
            ArrayList<com.droid.dx.ssa.SsaInsn> insnList = predBlock.getInsns();

            /**
             * If the predecessor block has a check-cast, it will be the last
             * instruction
             */
            com.droid.dx.ssa.SsaInsn checkCastInsn = insnList.get(insnList.size() - 1);
            if (checkCastInsn.getOpcode().getOpcode() != com.droid.dx.rop.code.RegOps.CHECK_CAST) {
                continue;
            }
            ...
            if (moveRopReg != checkRopReg && !hasExceptionHandlers) {
                ((com.droid.dx.ssa.NormalSsaInsn) checkCastInsn).changeOneSource(0,
    ...
    */
    private void handlePhiInsns() {
        for (com.droid.dx.ssa.PhiInsn insn : phiInsns) {
            processPhiInsn(insn);
        }
    }
    ...
    private void analyzeInstructions() {
        ssaMeth.forEachInsn(new com.droid.dx.ssa.SsaInsn.Visitor() {
            ...
            /** {@inheritDoc} */
            public void visitPhiInsn(com.droid.dx.ssa.PhiInsn insn) {
                processInsn(insn);
            }
             ...
             */
            private void processInsn(com.droid.dx.ssa.SsaInsn insn) {
                RegisterSpec assignment;
                assignment = insn.getLocalAssignment();

                if (assignment != null) {
                    com.droid.dx.rop.code.LocalItem local = assignment.getLocalItem();
                ...
                if (insn instanceof com.droid.dx.ssa.NormalSsaInsn) {
                    if (insn.getOpcode().getOpcode() ==
                            com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
                        moveResultPseudoInsns.add((com.droid.dx.ssa.NormalSsaInsn) insn);
                    } else if (com.droid.dx.ssa.Optimizer.getAdvice().requiresSourcesInOrder(
                            insn.getOriginalRopInsn().getOpcode(),
                            insn.getSources())) {
                        invokeRangeInsns.add((com.droid.dx.ssa.NormalSsaInsn) insn);
                    }
                } else if (insn instanceof com.droid.dx.ssa.PhiInsn) {
                    phiInsns.add((com.droid.dx.ssa.PhiInsn) insn);
                }
     ...
     */
    private void adjustAndMapSourceRangeRange(com.droid.dx.ssa.NormalSsaInsn insn) {
        ...
        for (int i = 0; i < szSources; i++) {
            ...
            com.droid.dx.rop.code.LocalItem localItem = getLocalItemForReg(sourceReg);
     ...
     */
    private int findRangeAndAdjust(com.droid.dx.ssa.NormalSsaInsn insn) {
        RegisterSpecList sources = insn.getSources();
        int szSources = sources.size();
     ...
     */
    private int findAnyFittingRange(com.droid.dx.ssa.NormalSsaInsn insn, int rangeLength,
     ...
     */
    private int fitPlanForRange(int ropReg, com.droid.dx.ssa.NormalSsaInsn insn,
            int[] categoriesForIndex, BitSet outMovesRequired) {
        RegisterSpecList sources = insn.getSources();
        int szSources = sources.size();
        int fitWidth = 0;
        com.droid.dx.util.IntSet liveOut = insn.getBlock().getLiveOutRegs();
     ...
     */
    RegisterSpecList ssaSetToSpecs(com.droid.dx.util.IntSet ssaSet) {
     ...
     */
    private com.droid.dx.rop.code.LocalItem getLocalItemForReg(int ssaReg) {
        for (Map.Entry<com.droid.dx.rop.code.LocalItem, ArrayList<RegisterSpec>> entry :
                 localVariables.entrySet()) {
     ...
     */
    private void processPhiInsn(com.droid.dx.ssa.PhiInsn insn) {
        ...
        for (int i = 0; i < sourcesSize; i++) {
            RegisterSpec source = sources.get(i);
            com.droid.dx.ssa.SsaInsn def = ssaMeth.getDefinitionForRegister(source.getReg());
