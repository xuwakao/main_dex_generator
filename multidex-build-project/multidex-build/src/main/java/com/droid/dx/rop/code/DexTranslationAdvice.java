...
public final class DexTranslationAdvice
    ...
    public boolean hasConstantOperation(Rop opcode,
        ...
        if (sourceA.getType() != com.droid.dx.rop.type.Type.INT) {
            return false;
        }

        // Return false if second source isn't a constant
        if (! (sourceB.getTypeBearer() instanceof com.droid.dx.rop.cst.CstInteger)) {
            // Except for rsub-int (reverse sub) where first source is constant
            if (sourceA.getTypeBearer() instanceof com.droid.dx.rop.cst.CstInteger &&
                    opcode.getOpcode() == RegOps.SUB) {
                com.droid.dx.rop.cst.CstInteger cst = (com.droid.dx.rop.cst.CstInteger) sourceA.getTypeBearer();
        ...
        com.droid.dx.rop.cst.CstInteger cst = (com.droid.dx.rop.cst.CstInteger) sourceB.getTypeBearer();
        ...
        switch (opcode.getOpcode()) {
            ...
            case RegOps.SUB:
                com.droid.dx.rop.cst.CstInteger cst2 = com.droid.dx.rop.cst.CstInteger.make(-cst.getValue());
