 ...
 * ignores the distinction between {@link com.droid.dx.rop.type.Type} and {@link
 * TypeBearer}.</p>
 */
public abstract class BaseMachine implements Machine {
    ...
    private com.droid.dx.rop.type.Type auxType;
    ...
    public void popArgs(Frame frame, Prototype prototype) {
        com.droid.dx.rop.type.StdTypeList types = prototype.getParameterTypes();
        int size = types.size();
        ...
        for (int i = 0; i < size; i++) {
            if (! com.droid.dx.cf.code.Merger.isPossiblyAssignableFrom(types.getType(i), args[i])) {
                throw new com.droid.dx.cf.code.SimException("at stack depth " + (size - 1 - i) +
    ...
    public final void popArgs(Frame frame, com.droid.dx.rop.type.Type type) {
        // Use the above method to do the actual popping...
        popArgs(frame, 1);

        // ...and then verify the popped type.
        if (! com.droid.dx.cf.code.Merger.isPossiblyAssignableFrom(type, args[0])) {
            throw new com.droid.dx.cf.code.SimException("expected type " + type.toHuman() +
                    " but found " + args[0].getType().toHuman());
        }
    }

    /** {@inheritDoc} */
    public final void popArgs(Frame frame, com.droid.dx.rop.type.Type type1, com.droid.dx.rop.type.Type type2) {
        // Use the above method to do the actual popping...
        popArgs(frame, 2);

        // ...and then verify the popped types.

        if (! com.droid.dx.cf.code.Merger.isPossiblyAssignableFrom(type1, args[0])) {
            throw new com.droid.dx.cf.code.SimException("expected type " + type1.toHuman() +
                    " but found " + args[0].getType().toHuman());
        }

        if (! com.droid.dx.cf.code.Merger.isPossiblyAssignableFrom(type2, args[1])) {
            throw new com.droid.dx.cf.code.SimException("expected type " + type2.toHuman() +
    ...
    public final void popArgs(Frame frame, com.droid.dx.rop.type.Type type1, com.droid.dx.rop.type.Type type2,
        ...
        if (! com.droid.dx.cf.code.Merger.isPossiblyAssignableFrom(type3, args[2])) {
            throw new com.droid.dx.cf.code.SimException("expected type " + type3.toHuman() +
    ...
    /** {@inheritDoc} */
    public final void auxType(com.droid.dx.rop.type.Type type) {
        auxType = type;
    }
    ...
    /** {@inheritDoc} */
    public final void localTarget(int idx, com.droid.dx.rop.type.Type type, com.droid.dx.rop.code.LocalItem local) {
     ...
     */
    protected final com.droid.dx.rop.type.Type getAuxType() {
        return auxType;
    }
    ...
    protected final RegisterSpec getLocalTarget(boolean isMove) {
        ...
        if (resultCount != 1) {
            throw new com.droid.dx.cf.code.SimException("local target with " +
                    ((resultCount == 0) ? "no" : "multiple") + " results");
        }

        TypeBearer result = results[0];
        com.droid.dx.rop.type.Type resultType = result.getType();
        com.droid.dx.rop.type.Type localType = localTarget.getType();
        ...
        if (! com.droid.dx.cf.code.Merger.isPossiblyAssignableFrom(localType, resultType)) {
            // The result and local types are inconsistent. Complain!
            throwLocalMismatch(resultType, localType);
            return null;
        }

        if (localType == com.droid.dx.rop.type.Type.OBJECT) {
            /*
    ...
    protected final int resultCount() {
        if (resultCount < 0) {
            throw new com.droid.dx.cf.code.SimException("results never set");
        }
    ...
    public static void throwLocalMismatch(TypeBearer found,
            TypeBearer local) {
        throw new com.droid.dx.cf.code.SimException("local variable type mismatch: " +
