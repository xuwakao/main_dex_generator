...
public final class BasicBlocker implements BytecodeArray.Visitor {
     ...
     */
    private final com.droid.dx.cf.code.ByteCatchList[] catchLists;
    ...
    private BasicBlocker(ConcreteMethod method) {
        ...
        workSet = com.droid.dx.util.Bits.makeBitSet(sz);
        liveSet = com.droid.dx.util.Bits.makeBitSet(sz);
        blockSet = com.droid.dx.util.Bits.makeBitSet(sz);
        targetLists = new IntList[sz];
        catchLists = new com.droid.dx.cf.code.ByteCatchList[sz];
        previousOffset = -1;
    }
    ...
    /** {@inheritDoc} */
    public void visitNoArgs(int opcode, int offset, int length, com.droid.dx.rop.type.Type type) {
        switch (opcode) {
            ...
            case ByteOps.IREM: {
                 ...
                 */
                visitCommon(offset, length, true);
                if ((type == com.droid.dx.rop.type.Type.INT) || (type == com.droid.dx.rop.type.Type.LONG)) {
                    visitThrowing(offset, length, true);
                }
                break;
            }
    ...
    public void visitConstant(int opcode, int offset, int length,
        ...
        if ((cst instanceof com.droid.dx.rop.cst.CstMemberRef) || (cst instanceof com.droid.dx.rop.cst.CstType) ||
            (cst instanceof com.droid.dx.rop.cst.CstString)) {
            /*
    ...
    public void visitNewarray(int offset, int length, com.droid.dx.rop.cst.CstType type,
            ArrayList<com.droid.dx.rop.cst.Constant> intVals) {
        visitCommon(offset, length, true);
        visitThrowing(offset, length, true);
    }
    ...
    private ByteBlockList getBlockList() {
        ...
        for (int at = 0, next; /*at*/; at = next) {
            next = com.droid.dx.util.Bits.findFirst(blockSet, at + 1);
            if (next < 0) {
                break;
            }

            if (com.droid.dx.util.Bits.get(liveSet, at)) {
                /*
                ...
                int targetsAt = -1;
                com.droid.dx.cf.code.ByteCatchList blockCatches;
                ...
                if (targets == null) {
                    ...
                    blockCatches = com.droid.dx.cf.code.ByteCatchList.EMPTY;
                } else {
    ...
    private void doit() {
        BytecodeArray bytes = method.getCode();
        com.droid.dx.cf.code.ByteCatchList catches = method.getCatches();
        ...
        com.droid.dx.util.Bits.set(blockSet, 0);
         ...
         */
        while (!com.droid.dx.util.Bits.isEmpty(workSet)) {
            try {
            ...
            for (int i = 0; i < catchSz; i++) {
                com.droid.dx.cf.code.ByteCatchList.Item item = catches.get(i);
                int start = item.getStartPc();
                int end = item.getEndPc();
                if (com.droid.dx.util.Bits.anyInRange(liveSet, start, end)) {
                    com.droid.dx.util.Bits.set(blockSet, start);
                    com.droid.dx.util.Bits.set(blockSet, end);
    ...
    private void addWorkIfNecessary(int offset, boolean blockStart) {
        if (!com.droid.dx.util.Bits.get(liveSet, offset)) {
            com.droid.dx.util.Bits.set(workSet, offset);
        }

        if (blockStart) {
            com.droid.dx.util.Bits.set(blockSet, offset);
        }
    }
    ...
    private void visitCommon(int offset, int length, boolean nextIsLive) {
        com.droid.dx.util.Bits.set(liveSet, offset);
        ...
        if (nextIsLive) {
             ...
             * live.
             */
            com.droid.dx.util.Bits.set(blockSet, offset + length);
        }
    }
    ...
    private void visitThrowing(int offset, int length, boolean nextIsLive) {
        ...
        com.droid.dx.cf.code.ByteCatchList catches = method.getCatches().listFor(offset);
