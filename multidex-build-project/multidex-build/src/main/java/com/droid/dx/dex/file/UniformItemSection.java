 ...
 * {@link com.droid.dx.dex.file.Item} objects. Each of the items must have the same size in
 * the output.
 */
public abstract class UniformItemSection extends Section {
     ...
     */
    public UniformItemSection(String name, com.droid.dx.dex.file.DexFile file, int alignment) {
    ...
    public final int writeSize() {
        Collection<? extends com.droid.dx.dex.file.Item> items = items();
        int sz = items.size();
     ...
     * com.droid.dx.dex.file.IndexedItem} instances.
     *
     * @param cst {@code non-null;} constant to look for
     * @return {@code non-null;} the corresponding item found in this instance
     */
    public abstract com.droid.dx.dex.file.IndexedItem get(Constant cst);

    /** {@inheritDoc} */
    @Override
    protected final void prepare0() {
        com.droid.dx.dex.file.DexFile file = getFile();

        orderItems();

        for (com.droid.dx.dex.file.Item one : items()) {
    ...
    /** {@inheritDoc} */
    @Override
    public final int getAbsoluteItemOffset(com.droid.dx.dex.file.Item item) {
        /*
         * Since all items must be the same size, we can use the size
         * of the one we're given to calculate its offset.
         */
        com.droid.dx.dex.file.IndexedItem ii = (com.droid.dx.dex.file.IndexedItem) item;
