...
public class SCCP {
        ...
        if (opcode.getBranchingness() == Rop.BRANCH_IF) {
            ...
            if (cA != null && sources.size() == 1) {
                switch (((TypedConstant) cA).getBasicType()) {
                    case Type.BT_INT:
                        constantBranch = true;
                        int vA = ((com.droid.dx.rop.cst.CstInteger) cA).getValue();
                        switch (opcode.getOpcode()) {
                            ...
                            case com.droid.dx.rop.code.RegOps.IF_NE:
                                constantSuccessor = (vA != 0);
                                break;
                            case com.droid.dx.rop.code.RegOps.IF_LT:
                                constantSuccessor = (vA < 0);
                                break;
                            case com.droid.dx.rop.code.RegOps.IF_GE:
                                constantSuccessor = (vA >= 0);
                                break;
                            case com.droid.dx.rop.code.RegOps.IF_LE:
                                constantSuccessor = (vA <= 0);
                                break;
                            case com.droid.dx.rop.code.RegOps.IF_GT:
                                constantSuccessor = (vA > 0);
                                break;
                            default:
                ...
                switch (((TypedConstant) cA).getBasicType()) {
                    case Type.BT_INT:
                        constantBranch = true;
                        int vA = ((com.droid.dx.rop.cst.CstInteger) cA).getValue();
                        int vB = ((com.droid.dx.rop.cst.CstInteger) cB).getValue();
                        switch (opcode.getOpcode()) {
                            case com.droid.dx.rop.code.RegOps.IF_EQ:
    ...
    private Constant simulateMath(SsaInsn insn, int resultType) {
        ...
        switch (resultType) {
            case Type.BT_INT:
                ...
                switch (opcode) {
                    case com.droid.dx.rop.code.RegOps.ADD:
                        vR = vA + vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.SUB:
                        ...
                        break;
                    case com.droid.dx.rop.code.RegOps.MUL:
                        vR = vA * vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.DIV:
                        if (vB == 0) {
                            skip = true;
                        ...
                        break;
                    case com.droid.dx.rop.code.RegOps.AND:
                        vR = vA & vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.OR:
                        vR = vA | vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.XOR:
                        vR = vA ^ vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.SHL:
                        vR = vA << vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.SHR:
                        vR = vA >> vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.USHR:
                        vR = vA >>> vB;
                        break;
                    case com.droid.dx.rop.code.RegOps.REM:
                        if (vB == 0) {
                            skip = true;
                            vR = 0; // just to hide a warning
                        } else {
                ...
                return skip ? null : com.droid.dx.rop.cst.CstInteger.make(vR);
    ...
    private void simulateStmt(SsaInsn insn) {
        ...
        if (result == null) {
            ...
            if (opcode == com.droid.dx.rop.code.RegOps.DIV || opcode == com.droid.dx.rop.code.RegOps.REM) {
        ...
        switch (opcode) {
            ...
            case com.droid.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO: {
    ...
    private void replaceBranches() {
        for (SsaInsn insn : branchWorklist) {
            ...
            block.replaceLastInsn(new PlainInsn(com.droid.dx.rop.code.Rops.GOTO,
