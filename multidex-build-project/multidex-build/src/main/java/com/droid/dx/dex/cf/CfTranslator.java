...
package com.droid.dx.dex.cf;

import com.droid.dx.cf.code.ConcreteMethod;
import com.droid.dx.cf.code.Ropper;
import com.droid.dx.cf.iface.Field;
...
import com.droid.dx.dex.code.DalvCode;
import com.droid.dx.dex.code.PositionList;
import com.droid.dx.dex.code.RopTranslator;
import com.droid.dx.dex.file.ClassDefItem;
import com.droid.dx.dex.file.DexFile;
import com.droid.dx.dex.file.EncodedField;
import com.droid.dx.dex.file.EncodedMethod;
import com.droid.dx.dex.file.FieldIdsSection;
import com.droid.dx.dex.file.MethodIdsSection;
import com.droid.dx.rop.annotation.Annotations;
import com.droid.dx.rop.annotation.AnnotationsList;
import com.droid.dx.rop.code.DexTranslationAdvice;
import com.droid.dx.rop.code.LocalVariableExtractor;
import com.droid.dx.rop.code.LocalVariableInfo;
import com.droid.dx.rop.code.RopMethod;
import com.droid.dx.rop.code.TranslationAdvice;
import com.droid.dx.rop.cst.Constant;
import com.droid.dx.rop.cst.ConstantPool;
import com.droid.dx.rop.cst.CstBaseMethodRef;
import com.droid.dx.rop.cst.CstBoolean;
import com.droid.dx.rop.cst.CstByte;
import com.droid.dx.rop.cst.CstChar;
import com.droid.dx.rop.cst.CstEnumRef;
import com.droid.dx.rop.cst.CstFieldRef;
import com.droid.dx.rop.cst.CstInteger;
import com.droid.dx.rop.cst.CstInterfaceMethodRef;
import com.droid.dx.rop.cst.CstShort;
import com.droid.dx.rop.cst.CstString;
import com.droid.dx.rop.cst.CstType;
import com.droid.dx.rop.cst.TypedConstant;
import com.droid.dx.rop.type.Type;
import com.droid.dx.rop.type.TypeList;
import com.droid.dx.ssa.Optimizer;
...
public class CfTranslator {
     ...
     */
    public static ClassDefItem translate(com.droid.dx.cf.direct.DirectClassFile cf, byte[] bytes,
                                         com.droid.dx.dex.cf.CfOptions cfOptions, DexOptions dexOptions, DexFile dexFile) {
        try {
            return translate0(cf, bytes, cfOptions, dexOptions, dexFile);
        } catch (RuntimeException ex) {
            String msg = "...while processing " + cf.getFilePath();
            throw com.droid.dex.util.ExceptionWithContext.withContext(ex, msg);
        }
    }
     ...
     * @return {@code non-null;} the translated class
     */
    private static ClassDefItem translate0(com.droid.dx.cf.direct.DirectClassFile cf, byte[] bytes,
                                           com.droid.dx.dex.cf.CfOptions cfOptions, DexOptions dexOptions, DexFile dexFile) {
        ...
        int classAccessFlags = cf.getAccessFlags() & ~com.droid.dx.rop.code.AccessFlags.ACC_SUPER;
        ...
        Annotations classAnnotations =
            com.droid.dx.dex.cf.AttributeTranslator.getClassAnnotations(cf, cfOptions);
        ...
        for (int i = 0; i < constantPoolSize; i++) {
            ...
            if (constant instanceof com.droid.dx.rop.cst.CstMethodRef) {
    ...
    private static void processFields(
            com.droid.dx.cf.direct.DirectClassFile cf, ClassDefItem out, DexFile dexFile) {
        CstType thisClass = cf.getThisClass();
        ...
        for (int i = 0; i < sz; i++) {
            ...
            try {
                ...
                if (com.droid.dx.rop.code.AccessFlags.isStatic(accessFlags)) {
                ...
                Annotations annotations =
                    com.droid.dx.dex.cf.AttributeTranslator.getAnnotations(one.getAttributes());
     ...
     * @param dexFile {@code non-null;} dex output
     */
    private static void processMethods(com.droid.dx.cf.direct.DirectClassFile cf, com.droid.dx.dex.cf.CfOptions cfOptions,
        ...
        for (int i = 0; i < sz; i++) {
            ...
            try {
                com.droid.dx.rop.cst.CstMethodRef meth = new com.droid.dx.rop.cst.CstMethodRef(thisClass, one.getNat());
                int accessFlags = one.getAccessFlags();
                boolean isStatic = com.droid.dx.rop.code.AccessFlags.isStatic(accessFlags);
                boolean isPrivate = com.droid.dx.rop.code.AccessFlags.isPrivate(accessFlags);
                boolean isNative = com.droid.dx.rop.code.AccessFlags.isNative(accessFlags);
                boolean isAbstract = com.droid.dx.rop.code.AccessFlags.isAbstract(accessFlags);
                ...
                if (isNative || isAbstract) {
                    ...
                    if (cfOptions.optimize &&
                        ...
                        if (cfOptions.statistics) {
                            com.droid.dx.dex.cf.CodeStatistics.updateRopStatistics(
                                    nonOptRmeth, rmeth);
                        }
                    }
                ...
                if (com.droid.dx.rop.code.AccessFlags.isSynchronized(accessFlags)) {
                    accessFlags |= com.droid.dx.rop.code.AccessFlags.ACC_DECLARED_SYNCHRONIZED;
                     ...
                     */
                    if (!isNative) {
                        accessFlags &= ~com.droid.dx.rop.code.AccessFlags.ACC_SYNCHRONIZED;
                    }
                }

                if (isConstructor) {
                    accessFlags |= com.droid.dx.rop.code.AccessFlags.ACC_CONSTRUCTOR;
                }

                TypeList exceptions = com.droid.dx.dex.cf.AttributeTranslator.getExceptions(one);
                EncodedMethod mi =
                ...
                Annotations annotations =
                    com.droid.dx.dex.cf.AttributeTranslator.getMethodAnnotations(one);
                if (annotations.size() != 0) {
                    out.addMethodAnnotations(meth, annotations, dexFile);
                }

                AnnotationsList list =
                    com.droid.dx.dex.cf.AttributeTranslator.getParameterAnnotations(one);
                if (list.size() != 0) {
     ...
     */
    private static void updateDexStatistics(com.droid.dx.dex.cf.CfOptions cfOptions, DexOptions dexOptions,
        ...
        com.droid.dx.dex.cf.CodeStatistics.updateDexStatistics(nonOptCode, optCode);
        com.droid.dx.dex.cf.CodeStatistics.updateOriginalByteCount(originalByteCount);
    }
}
