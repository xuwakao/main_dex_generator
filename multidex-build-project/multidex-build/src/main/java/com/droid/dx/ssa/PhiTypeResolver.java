...
import com.droid.dx.cf.code.Merger;
import com.droid.dx.rop.code.LocalItem;
...
public class PhiTypeResolver {
    ...
    private void run() {
        ...
        for (int reg = 0; reg < regCount; reg++) {
            com.droid.dx.ssa.SsaInsn definsn = ssaMeth.getDefinitionForRegister(reg);
        ...
        while ( 0 <= (reg = worklist.nextSetBit(0))) {
             ...
             */
            com.droid.dx.ssa.PhiInsn definsn = (com.droid.dx.ssa.PhiInsn)ssaMeth.getDefinitionForRegister(reg);
            ...
            if (resolveResultType(definsn)) {
                ...
                List<com.droid.dx.ssa.SsaInsn> useList = ssaMeth.getUseListForRegister(reg);

                int sz = useList.size();
                for (int i = 0; i < sz; i++ ) {
                    com.droid.dx.ssa.SsaInsn useInsn = useList.get(i);
                    RegisterSpec resultReg = useInsn.getResult();
                    if (resultReg != null && useInsn instanceof com.droid.dx.ssa.PhiInsn) {
     ...
     */
    boolean resolveResultType(com.droid.dx.ssa.PhiInsn insn) {
